...
def check_create_validations():
    ...
    if alert_permission == "Yes" and not emails:
        st.warning("Please input invitee emails")
        return False
    return True
...
if alert_permission == "Yes":
    ...
    alert = st.selectbox(
        "Send Alert..", alert_options, index=None, placeholder="Notification Alert Time"
    )
    if alert:
        if alert == "At event's time":
            alert_time = 0
            alert_unit = "minutes"
        elif alert == "Custom":
            ...
            with col2:
                alert_unit = st.selectbox(
                    "Minutes or Hours", ("minutes", "hours"), index=None
                )
        else:
            alert_time = int(re.findall(r'\d+', alert)[0])
            if "minutes" in alert:
                alert_unit = "minutes"
            elif "hour" in alert:
                alert_unit = "hours"
...
if st.button("Add The Task"):
    ...
    if check_create_validations():
        data = {
            "title": task_title,
            "color": "#FFBD45",
            "start": start.strftime("%Y-%m-%d")
            + "T"
            + time_range[0].strftime("%H:%M:%S"),
